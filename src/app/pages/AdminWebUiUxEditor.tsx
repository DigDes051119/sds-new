...
import { Plus, Trash2, Edit2, Check, Save, X, Image, Loader2, Camera, ChevronUp, ChevronDown, Type, Copy, Video, Play, Film } from "lucide-react";
...
export function AdminWebUiUxEditor() {
  ...
  const handleProjectDragStart = (e: React.DragEvent, index: number, project: any) => {
    if (isReadOnly) return;
    setDraggedProjectIndex(index);
    const dragPayload = JSON.stringify({ index, itemId: project.id, sourceType: "webUiUx", itemName: project.name });
    e.dataTransfer.setData("application/json", dragPayload);
    e.dataTransfer.setData("text/plain", dragPayload);
    (window as any).__draggedAdminCatalogItem = { itemId: project.id, sourceType: "webUiUx", itemName: project.name };
  };
  ...
  const handleProjectDrop = async (e: React.DragEvent, targetIndex: number) => {
    e.preventDefault();
    let sourceIndex = draggedProjectIndex;
    if (sourceIndex === null) {
      try {
        const raw = e.dataTransfer.getData("application/json") || e.dataTransfer.getData("text/plain");
        if (raw) {
          const parsed = JSON.parse(raw);
          if (typeof parsed.index === "number") sourceIndex = parsed.index;
          else if (!isNaN(parseInt(raw, 10))) sourceIndex = parseInt(raw, 10);
        }
      } catch {}
    }
    ...
    try {
      ...
      langs.forEach(lang => {
        const items = newTranslations[lang].webUiUx.items;
      ...
      await logAdminAction(
        "Управление проектами WEB / UI UX",
        "Сортировка проектов",
  ...
  const handleMoveProject = async (index: number, direction: "up" | "down") => {
    ...
    try {
      ...
      if (realSourceIdx !== -1 && realTargetIdx !== -1) {
        langs.forEach(lang => {
          const arr = newTranslations[lang].webUiUx.items;
  ...
  const startEditing = (id: string) => {
    ...
    const projRu = translations.ru.webUiUx.items.find((p: any) => p.id === id) || {};
    const projEn = translations.en.webUiUx.items.find((p: any) => p.id === id) || {};
    const projKg = translations.kg.webUiUx.items.find((p: any) => p.id === id) || {};
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const existingCreatedAt = editingId
        ? (newTranslations.ru.webUiUx.items.find((p: any) => p.id === editingId)?.createdAt || null)
      ...
      if (editingId) {
        ...
        newTranslations.ru.webUiUx.items.push(listEntryRu);
        newTranslations.en.webUiUx.items.push(listEntryEn);
        newTranslations.kg.webUiUx.items.push(listEntryKg);
  ...
  const handleDuplicate = async (id: string) => {
    ...
    try {
      setTranslating(true);
      const res = await cmsService.duplicateProject(id, "webUiUx");
      if (res.success) {
        await logAdminAction(
          "Управление проектами WEB / UI UX",
  ...
  const rawProjects = translations.ru.webUiUx.items;
              ...
              {filteredProjects.map((project: any) => {
                ...
                return (
                  <div
                    ...
                    onDragStart={(e) => handleProjectDragStart(e, realIdx, project)}
                    ...
                    title="Зажмите и перетащите в другой раздел в меню слева или для изменения порядка"
                  >
              ...
              <div className="px-6 py-10 bg-[#fafaf6] space-y-10 ">
                  ...
                  return (
                    <div key={blockIdx} className="space-y-3 bg-[#fafaf6] p-4 rounded-3xl border border-black/[0.04] w-full">
                      ...
                      <motion.div
                        ...
                        title="Зажмите и тащите вверх/вниз для сортировки блоков • Сюда можно перетащить фото или видео"
                      >
                        <div className={`grid gap-3 ${getGridColsClass(totalGridElements)}`}>
                          ...
                          {/* Add Media Slot inside the block grid */}
                          {hasAddSlot && (
                            <div
                              onDragOver={(e) => e.preventDefault()}
                              ...
                              title="Добавить фото или видео в этот блок"
                            >
                              <input
                      ...
                      {formCollageBlocks.length > 1 && blockIdx < formCollageBlocks.length - 1 && (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 my-2">
                          <button
                            ...
                            title="Вставить блок коллажа • Сюда можно перетащить медиа"
                          >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4">
                  <button
                    ...
                    title="Добавить блок коллажа • Сюда можно перетащить медиа"
                  >
